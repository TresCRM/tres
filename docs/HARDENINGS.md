...
- [x] **[P1]** Add refresh-token rotation with reuse detection (invalidate family on replay). — **VERIFIED ALREADY DONE (audit 2026-08-30): auth.ts:580-600 rotates on refresh and detects reuse via replacedByHash, invalidating the family on replay**
...
- [x] **[P0]** No CSRF/nonce/replay protection on widget submissions. Since the token is public, at minimum: server-side rate-limit per token+IP; add optional invisible CAPTCHA (Turnstile) toggle in widget config. — **PARTIAL 2026-08-27/28: the server-side half is done — widgetTicketLimiter caps tickets at 5/hour per widget token and widgetTokenLimiter caps overall widget traffic at 1000/hour per token. Not done: keying the throttle on token+IP jointly, and the optional Turnstile toggle. Both remain open** — **DONE 2026-08-30: widget ticket creation is now bounded by two layered limits — 5/hour per token+address (so one abusive visitor cannot spend a tenant's whole allowance and lock out genuine customers, which a token-only budget did) and 60/hour per token in aggregate (the bound that survives address rotation). Plus 1000 requests/hour per token across all widget endpoints. Turnstile remains unimplemented and is the one part of this item still outstanding**
...
- [x] **[P0]** Fail build on any test failure or axe violation of severity ≥ Serious. — **DONE 2026-08-29: the widget a11y spec asserts the axe scan yields no serious or critical violations, so a regression fails that spec and therefore the Widget E2E job and the build. Both E2E suites are required checks on the PR**
...
- [x] **[P0]** `tickets.spec.ts` — create ticket, list filter, assign, add comment (internal + public), close, reopen, delete. — **DONE 2026-08-29 (21 specs) apart from delete: create with validation and permission cases, list plus status and priority filters, fetch by id, public and internal comments, assignment, close, reopen, the state machine refusing OPEN->RESOLVED while accepting OPEN->IN_PROGRESS, and cross-tenant access returning 404. Delete is now covered too: a DELETE /:id endpoint was added 2026-08-30, gated on a new TICKET_DELETE permission held only by OWNER and ADMIN, with soft-delete semantics**
- [x] **[P0]** `customers.spec.ts` — CRUD + search + tag. — **DONE 2026-08-29 (25 specs) apart from tagging: create/read/update/delete with validation, duplicate-email rejection, list limit, search by name, company and email including case-insensitivity and a regex-metacharacter case proving the query is escaped, the full permission split, and two cross-tenant cases (another tenant's customer is unreadable, undeletable and absent from search). Tagging is now covered: a tags field was added to Customer 2026-08-30 with normalisation, filtering and search. NOTE the suite also pins that AGENT holds CUSTOMER_READ only; customer writes belong to OWNER/ADMIN**
- [x] **[P0]** `billing.spec.ts` — view plan, start upgrade → Paystack redirect (mocked), verify success flow. — **DONE 2026-08-29 (17 specs): plan catalogue, current subscription including tenant scoping, checkout returning a Paystack authorization URL, verify accepting a successful transaction, invoices, and the BILLING_MANAGE/BILLING_READ permission split. The mock is a stub Paystack served by the harness with the API pointed at it via a new PAYSTACK_BASE_URL override — the base URL was hardcoded, so checkout could only ever answer 503 (no provider) or call the live service**
